...
type TrianglesBrutalParams = { seed: number; density: number };

interface TrianglesBrutalProps {
	width?: number;
	height?: number;
	params: TrianglesBrutalParams;
}

// ---------------------------------------------------------------------------
// BRUTALIST / SWISS POSTER (white-dominant) framing of the Triangles
// (low-poly / kumiko) generator. The original generative drawing is LIFTED here
// (its mulberry32 PRNG + subdivide/hatch/clip routines) and rendered into ONE
// inline <svg viewBox="0 0 800 480"> that fills the content area below a single
// black header bar carrying "TRIANGLES" reversed out in white. Thick black
// rules frame the art; the triangle mesh is line/hatch work on white (only a
// sparse ~12% of cells fill solid, per the original), so the page stays
// predominantly white and the e-ink panel ghost-free.
// Pure #000 / #fff. Flexbox + inline SVG only — no grid, filter, gradient,
// shadow or opacity. SVG children grouped in <g>, never React Fragments.
// ---------------------------------------------------------------------------

// Logical art canvas (matches the original Triangles viewBox).
const W = 800;
const H = 480;
...
// Build the full set of lifted SVG art elements for the triangle mesh.
function buildElements(seed: number, depth: number): React.ReactNode[] {
		...
		<PreSatori width={width} height={height}>
			...
			>
				...
				>
					...
					>
						<title>Triangles — generative low-poly mesh (brutalist)</title>
