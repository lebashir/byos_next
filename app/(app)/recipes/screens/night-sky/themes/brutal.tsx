...
type NightSkyBrutalParams = {
	lat: number;
	lon: number;
	showLines: boolean;
};

interface NightSkyBrutalProps {
	width?: number;
	height?: number;
	params: NightSkyBrutalParams;
}

// ---------------------------------------------------------------------------
// BRUTALIST / SWISS POSTER (white-dominant) framing of the Night Sky star
// chart. The original star-chart math + drawing is LIFTED VERBATIM (bundled
// catalog, the J2000 GMST/LST computation, the alt/az projection in
// buildChart, the constellation LINES and star/cardinal rendering). It is drawn
// into ONE inline <svg> that fills the content area below a single black header
// bar carrying "NIGHT SKY" reversed out in white, with a small LAT/LON label.
// Thick black rules frame the chart; the sky is dots + thin lines on white, so
// the page stays predominantly white and the e-ink panel ghost-free.
...
export default function NightSkyBrutal({
	...
	// Chart geometry computed inside the content area (below the header).
	const PAD = 26;
	const R = (artH - 2 * PAD) / 2;
	const cx = width / 2;
	const cy = artH / 2;
		...
		<PreSatori width={width} height={height}>
			...
			>
				...
				>
					...
					{/* small lat/lon label reversed out at the right of the bar */}
					<div
				...
				>
					<svg
						width={width}
						height={artH}
						viewBox={`0 0 ${width} ${artH}`}
						xmlns="http://www.w3.org/2000/svg"
						style={{ display: "block", backgroundColor: "#fff" }}
					>
						<title>Night Sky star chart (brutalist)</title>
						{/* Paper background */}
						<rect x={0} y={0} width={width} height={artH} fill="#fff" />

						{/* Horizon circle (chart border) — thick, brutalist */}
						<circle
							cx={cx}
							cy={cy}
							r={R}
